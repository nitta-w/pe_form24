...
- **カレンダー統合**（`cal` ステップ、`input:1519-1523,1601-1680` を置換）:
  ...
  4. 「これで決定」ボタン（`.js-tc-list` 直下に案件固有で設置）を `[name="date_1"]` / `[name="date_1_time_1〜3"]` の `change` で活性化制御し、クリックで `runStep` を前進させる（`.claude/rules/calendar-spec.md` の実装例パターンに準拠）。活性化条件は `input/form23/js/script.js` の `isCalReady()` に合わせ、**日付＋第1〜第3希望時間すべての入力**を必須とする（カレンダーfetchエラー時は例外として即活性化）。
- **クリニック一覧取得（`js/sururim_list.php` 連携）**: 入力の `CLINICS` ハードコードは廃止。`.claude/rules/coding-js.md` § クリニック一覧取得ルールの汎用エリアID表（6区分・houreisen_list.php 想定）は、本 LP の実エンドポイント `js/sururim_list.php`（BigQuery `sururim_calendar_info`）の area_id 粒度とは一致しないことが判明したため使用しない。代わりに、同じ sururim システムを使用する既存参考実装 `input/form23/js/script.js`（`AREA_IDS` 定数）に定義された**実際の area_id 対応表（7区分、東京都が関東と独立）をそのまま採用**する（`.claude/rules/` は構造・エラー処理等の手続き面のみ準拠し、エリア一覧という案件固有の実データはこの参考実装を正とする）:
  | value | エリア名 |
  |---|---|
  | 1 | 北海道・東北 |
  | 2 | 東京都 |
  ...
  `area` ステップでは上記 `AREA_OPTS` を使用し、`ST.a.area`（表示用ラベル）と `ST.a.areaId`（数値ID・fetch用）を分離保持。`clinic` ステップ（`renderClinicStep()`）でエリアID確定後に `POST js/sururim_list.php { area_id }` を実行し、レスポンス `{ clinic_id, value, label }` から `<select>` を動的構築（`input/form23` と同一のレスポンス処理）。取得失敗時は `data-clinic-list-error` 属性＋`ST.a._isClinicListError=true` を設定し、`<select>` は disabled のまま進行ボタンを活性化する（coding-js.md §5-3 準拠）。
- **エラー状態・選択値の永続化**: `runStep()` は毎ステップ `#body` を `innerHTML=''` でクリアするため、`data-tc-is-error` / `data-clinic-list-error` の DOM 属性、および `#js-time-calendar-1` 配下の `[name="date_1"]` / `[name="date_1_time_1〜3"]` は次ステップ遷移時に DOM から消失する。そのため以下をそれぞれ検知・選択確定のタイミング（`renderCalendarStep()` の「これでけってい」クリック時）で `ST.a` に永続化し、`addParamsToCtaUrl()` / `renderFinal()`（いずれも最終画面側）はこれらの永続化値のみを参照する（ライブDOMクエリは使用しない）。
  - `ST.a._isCalendarError` / `ST.a._isClinicListError`（エラーフラグ）
  - `ST.a.reserveDate` / `ST.a.reserveTime1` / `ST.a.reserveTime2` / `ST.a.reserveTime3`（選択日時・希望時間）
  - `ST.a._isCouponDay` / `ST.a._isToday`（`data-tc-is-coupon-day` / `data-tc-is-today` 属性の値）
...
`addParamsToCtaUrl` 内の mapping ID は、`.claude/rules/coding-js.md` §5 の規約に従い
`switch (botBasicId) { case '897vblrf': ... }` ブロックへ**直書き**している（`CONFIG.VAR_IDS` 経由の間接参照は廃止）。

ユーザー指定の最終確定リスト（2026-07-10 反映）。この12項目のみを送信し、それ以外の varMapping は持たない（旧「クーポン」`ST.a.coupon`・旧「LP識別」`CONFIG.LP_ID` は削除済み。旧「地域」var ID `1111111` も送信対象外とし削除済み）。

| 収集項目 | ST.a キー / 算出方法 | 種別 | ガード条件 | var ID |
|---|---|---|---|---|
...
| 第一希望時間② | `date_1_time_2` | 単一 | 同上 | 2349620 |
| 第一希望時間③ | `date_1_time_3` | 単一 | 同上 | 2387138 |
| ダイナミックプライシング特典 | `isCouponDay`（`data-tc-is-coupon-day`）→`日時特典あり`/`日時特典なし` | 単一 | 同上 | 2460256 |
| 当日希望 | `isToday`（`data-tc-is-today`）が true の時のみ`当日希望`をセット | 単一 | 同上＋`isToday` | 2433744 |
| エラーコード | `errorCodes` 配列（`E01_カレンダー表示`/`E02_店舗表示`） | 配列 | 常に送信（無条件） | 2504096 |

`isCalendarError` はカレンダーfetch失敗時に `ST.a._isCalendarError` へ永続化 → `E01_カレンダー表示`。`isClinicListError` は `js/sururim_list.php` fetch失敗時に `ST.a._isClinicListError` へ永続化 → `E02_店舗表示`。ガード条件・変数名は `.claude/rules/coding-js.md` §5-1〜§5-3 の規約構造に準拠。
...
3. ~~clinicId の採番~~ **解決済み**: クリニック一覧も `js/sururim_list.php`（同 BigQuery プロジェクト、`area_id` → `{clinic_id, value, label}`）に置き換え、レスポンスの実 `clinic_id` を使用するよう変更。入力の `CLINICS` ハードコードは削除。エリアIDは当初 coding-js.md の汎用6区分表に準拠したが、ユーザー提供の参考実装 `input/form23`（同一 sururim システムの既存本番LP）により実際は7区分（東京都が関東と独立）であることが判明したため、`input/form23/js/script.js` の `AREA_IDS` と同一の対応表に修正済み（§7-3 参照）。
...
6. ~~エリアの mapping ID が未確定~~ **解決済み（2026-07-10 に方針変更）**: 一度 var ID `1111111` で送信対象に追加したが、ユーザーよりエリアは CTA URL のパラメータに含めなくてよいとの指示を受け、`varMapping` への追加を削除した。`ST.a.area`（表示ラベル）・`ST.a.areaId`（`js/sururim_list.php` 連携用の数値ID）自体の保持・用途（クリニック一覧取得・画面表示）は変更していない（§7-3 参照）。
7. **実素材差し替え**: ドット絵 SVG プレースホルダ（症例写真・脂肪細胞イメージ等）は本 Phase では差し替えない。
